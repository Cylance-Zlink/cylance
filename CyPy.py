...
class MyPrompt(Cmd):
    ...
    def do_deleteUser(self, args):
        ...
        if fields[0] == 'in':
            with open(fields[1]) as f:
                ...
                if len(lines) == 36:
                    print cy.delete_data("USERS", line)
        elif fields[0] == 'id':
            print cy.delete_data("USERS", fields[1])
    ...
    def do_getDevice(self, args):
        """Get Device by ID.\n getDevice id=<id>\n"""
        fields = args.split("=")
        print cy.get_data_by_id("DEVICES", fields[1])
    ...
    def do_deleteDevice(self, args):
        ...
        if fields[0] == 'in':
            with open(fields[1]) as f:
                line = f.readlines().strip()
                if len(lines) == 36:
                    print cy.delete_data("DEVICES", line)
        elif fields[0] == 'id':
